...
impl Robot {
        ...
        match current_cell {
            Cell::Science => {
                ...
                *count += 1;
                ...
                map.get_zone_at(self.x, self.y).map(|z| z.name.clone());
    ...
    pub fn automate_robot(&mut self, map: &Map, station: &Station, resources_revealed: bool) {
        ...
        let target = if self.inventory_count() >= self.inventory_capacity {
            ...
            if
                let Some(science_pos) = Self::find_nearest(
                    self.x,
                    self.y,
                    map,
                    Cell::Science,
                ...
                )
            {
                ...
                let Some(resource_pos) = Self::find_nearest_accessible_resource(
                    self.x,
                    self.y,
                    map,
                    resources_revealed
    ...
    pub fn find_nearest_accessible_resource(
        ...
        if
            let Some(pos) = Self::find_nearest_with_access_check(
                start_x,
                start_y,
                map,
                Cell::Mineral,
